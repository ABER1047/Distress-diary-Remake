		...
		if (moving_now == 1)
		{
			...
			else
			{
				...
				if (is_moving_item_outside == 0) //옮기고 있는 아이템이 인벤토리 칸 안인 경우
				{
					...
					var new_spr = nearsest_inv_variable_owner_ins.inv_info_spr_ind[new_y_pos][new_x_pos];
					var new_img = nearsest_inv_variable_owner_ins.inv_info_img_ind[new_y_pos][new_x_pos];
					var new_name = nearsest_inv_variable_owner_ins.inv_info_name[new_y_pos][new_x_pos];
					var new_stack_num = nearsest_inv_variable_owner_ins.inv_info_stack_num[new_y_pos][new_x_pos];
					
					//아이템이 겹치기 가능한 아이템인 경우
					if (x_pos == new_x_pos && y_pos == new_y_pos && new_spr == origin_spr  && new_img == origin_img && new_name == origin_name && new_stack_num != -4 && origin_stack_num != -4)
					{
