...
if (global.chat_activated)
{
	...
	if (keyboard_check_pressed(vk_enter))
	{
		if (chat_entering != "")
		{
			...
			{
				var commands = [ "/kill", "/dev", "/cm", "/debug", "/hitbox", "/tickrate", "/time", "/help", "/zoom", "/shadow", "/light", "/dmg", "/hunger", "/hydro", "/ts", "/mob", "/ef", "/arr", "/gq", "/vom", "/cp", "/invite", "/disconnect", "/connect", "/ds", "/explo", "/give", "/camera", "/hp", "/buff", "/np", "/roomall", "/pk", "/healall", "/igf" ];
				var command_desc = [ "자살하기", "개발자 모드 활성화/비활성화", "새로운 맵 생성", "디버그 모드 활성화/비활성화", "히트박스 활성화/비활성화", "지정된 수치만큼 틱레이트 설정", "지정된 수치만큼 시간 설정 (단위 : minute)", "명령어 가이드 표기", "카메라 줌 정도를 지정된 수차만큼 설정", "그림자 활성화/비활성화", "광원 활성화/비활성화", "내 플레이어에 지정된 수차만큼 데미지 입히기", "배고픔 게이지 소모", "수분 게이지 소모", "타일셋 변경", "몬스터 생성", "이펙트 생성", "화살 생성", "그래픽 퀄리티 설정", "몹 시점 표시", "퍼즐방 생성", "초대코드 복사", "서버 연결 해제", "서버 접속", "현재 룸의 모든 문 열기/닫기", "폭발 이펙트 생성", "아이템 생성", "카메라 위치 표시", "나 자신의 hp값 설정", "나 자신에게 버프/디버프 적용", "현재 내 위치 표시", "모든 룸 전부 지도에 표시", "팀킬 허용", "모든 플레이어 100% 회복", "화면 가림막 제거" ];
				for(var i = 0; i < array_length(commands); i++)
				{
					if (string_pos(commands[i],chat_entering))
					{
						...
						{
							var tmp_mob_obj_ind = [ obj_fireball, obj_wisp, obj_skull_head, obj_skeleton, obj_green_slime, obj_corruption_slime, obj_crimson_slime ];
