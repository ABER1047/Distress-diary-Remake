		...
		if (obj_id_player_only != 0) //서버 자신은 제외
		{
			...
			if (global.saved_players_xx[obj_id_player_only] == -5 || (x == global.saved_players_xx[obj_id_player_only] && y == global.saved_players_yy[obj_id_player_only]))
			{
				...
				if (global.saved_players_xx[obj_id_player_only] == -5)
				{
					...
					var pos = ds_list_find_index(other.clients, obj_id_player_only);
					ds_list_delete(other.clients, pos);
				...
				else
				{
					...
					//최적화 상 실제 핑의 3배만큼 곱해서 시간을 설정함 (= 토탈 6배임)
					is_there_any_player_need_to_check = floor(global.maximum_ping_acception/5*3);
					//show_message_log("- 비정상적으로 연결이 끊긴 플레이어를 감지함! ["+string(obj_id_player_only)+"]");
					
					//나갔나 아닌가 검사용으로 데이터 보내보기
					buffer_seek(tmp_buffer, buffer_seek_start, 0);
