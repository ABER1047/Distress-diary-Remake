	...
	for(var i = 0; i < tmp_owner.inv_height; i++)
	{
		for(var ii = 0; ii < tmp_owner.inv_width; ii++)
		{
			...
			if (tmp_spr_ind_real != -4)
			{
				...
				create_inv_item(tmp_spr_ind_real,tmp_owner.inv_info_img_ind[i][ii],tmp_owner.inv_info_name[i][ii],tmp_owner.inv_info_name_compressed[i][ii],tmp_owner.inv_info_stack_num[i][ii],tmp_owner.inv_info_max_stack_num[i][ii],id,ii,i,tmp_owner.inv_info_width[i][ii],tmp_owner.inv_info_height[i][ii],tmp_owner.inv_info_rotated[i][ii],tmp_weight,tmp_owner.inv_info_searched[i][ii]);
			}
		}
	}
