...
{
	var tmp_var_name = "x,y,image_xscale,image_yscale,count_connection,image_angle,rd_snap_rad,image_blend,max_reflection_num,count_reflection";
	var rd_dir = image_angle+ round(irandom_range(0,rd_snap_rad)*0.25) * choose(-4,4);

	var tmp_val = string(tx)+","+string(ty)+","+string(tmp_dis)+","+string(image_yscale)+","+string(count_connection+1)+","+string(rd_dir)+","+string(rd_snap_rad)+","+string(image_blend)+","+string(max_reflection_num)+","+string(count_reflection);
