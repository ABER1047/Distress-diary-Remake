...
if (!variable_instance_exists(id,"target"))
{
	...
	if (tmp_dis != 0 && count_connection < (max_reflection_num+1)*32)
	{
		...
		create_shockbolt(tx,ty,z,tmp_dis,image_yscale,rd_dir,count_reflection,max_reflection_num,image_blend,attacker_id,rd_snap_rad,dmg_info_arr,my_pos_xx,my_pos_yy,count_connection+1);
	}
	else
	{
		...
		else
		{
			...
			var tmp_xx = x+lengthdir_x(64,rd_dir);
			var tmp_yy = y+lengthdir_y(64,rd_dir);
			create_shockbolt(tmp_xx,tmp_yy,z,length,image_yscale,rd_dir,count_reflection+1,max_reflection_num,image_blend,attacker_id,rd_snap_rad,dmg_info_arr,my_pos_xx,my_pos_yy,count_connection+1);
		}
	}
}
